...
if [[ -d $location/build-input/enigma2 ]]; then
    ...
    cat $location/build-input/enigma2/*bouquet.* | grep -o '#SERVICE .*:0:.*:.*:.*:.*:.*:0:0:0' | sed -e 's/#SERVICE //g' -e 's/.*/\U&\E/' -e 's/:/_/g' | sort -u | while read serviceref ; do
        ...
        channelname=$(grep -i -A1 "${channelref[3]}:.*${channelref[6]}:.*${channelref[4]}:.*${channelref[5]}:.*:.*" <<< "$lamedb" | sed -n "2p" | iconv -f utf-8 -t ascii//translit 2>> $logfile | sed -e 's/^[ \t]*//' -e 's/|//g' -e 's/#34aPpwtmxeBiyZQ79uIB#//g')
    ...
    sort -t $'\t' -k 2,2 "$tempfile" | sed -e 's/\t/#34aPpwtmxeBiyZQ79uIB#|/g' | column -t -s $'#34aPpwtmxeBiyZQ79uIB#' | sed -e 's/|/  |  /g' > $file
    rm $tempfile
...
if [[ -d $location/build-input/tvheadend ]]; then
    ...
    for channelfile in $location/build-input/tvheadend/channel/config/* ; do
        ...
        channelname=$(grep 'svcname' $(find "$location/build-input/tvheadend" -type f -name $tvhservice) | sed -e 's/.*"svcname": "//g' -e 's/",//g' | iconv -f utf-8 -t ascii//translit 2>> $logfile | sed -e 's/^[ \t]*//' -e 's/|//g' -e 's/#34aPpwtmxeBiyZQ79uIB#//g')
...
if [[ -f $location/build-input/channels.conf ]]; then
    ...
    grep -o '.*:.*:.*:.*:.*:.*:.*:.*:.*:.*:.*:.*:0' $location/build-input/channels.conf | sort -u | while read channel ; do
        ...
        channelname=$(iconv -f utf-8 -t ascii//translit <<< "${channelname[0]}" 2>> $logfile | sed -e 's/^[ \t]*//' -e 's/|//g' -e 's/#34aPpwtmxeBiyZQ79uIB#//g')
